...
- **버전 확인**: 웹앱 URL을 브라우저로 GET → `{"version":"svN-..."}`. enrich는 토스트에 `[vN-...]`.
  - 현재 코드: enrich **v13-split**, submit **sv9-move**.
...
## 7. 현재 상태 / 마지막 작업
- 방금(v13-split / sv9-move — **그룹 정리 도구**): 디자인팀·기획팀 검토 반영 1단계.
  - 서버: `moveMember`(사람 다른 그룹 이동=대상 그룹원+본인에 공유 강제그룹 / 단독분리='__solo__'→오버라이드 분리Y). enrich가 오버라이드 **분리(단독)** 행을 자동병합 버킷에서 제외(강제그룹만 따름). 오버라이드 탭에 '분리(단독=Y)' 컬럼 추가.
  - 프론트: 관리자 **'그룹정리' 탭** 신설 — 의심목록(S1 같은전화 다른그룹 원클릭합치기 / S3 명단>제출) + 전체 그룹 목록 + 멤버별 select(이동/단독분리) + 그룹 체크 합치기.
  - 분리 데이터 없으면 enrich 결과 v12와 동일(78그룹/61,812,000) — 회귀 안전.
  - 다음(예정): 디자인팀 UI/UX Top7(방인원 자동값 접기, 용어 일상어화, 입금방식 질문형, 제출전 확인요약, 가독성 11→13px·대비, 총액↔분할 명시) — 사용자 화면 3탭. 기획팀 보고: 의심규칙 S2~S7 확장, 되돌리기, 합치기 미리보기.
- (직전 v12-roster — **동일 명단 자동 병합**): 같은 명단(이름 집합 완전일치)을 적은 사람들을 자동으로 한 그룹으로 → 전화/이메일 달라 쪼개지던 그룹(임성현 등)의 공동비용 중복 자동 해소. (강제그룹/합치기 수기 없이도) 회사 사본 78그룹/61,812,000원, 과병합 0(조형만 가족만 캠퍼스 다름). v11-mergefee 위에 얹음.
...
- 디자인팀 UI/UX 개선(Top7)·기획팀 의심규칙 확장은 아직 미구현(다음 차례).
