...
- **버전 확인**: 웹앱 URL을 브라우저로 GET → `{"version":"svN-..."}`. enrich는 토스트에 `[vN-...]`.
  - 현재 코드: enrich **v12-roster**, submit **sv7-mergefee**.
...
## 7. 현재 상태 / 마지막 작업
- 방금(v12-roster — **동일 명단 자동 병합**): 같은 명단(이름 집합 완전일치)을 적은 사람들을 자동으로 한 그룹으로 → 전화/이메일 달라 쪼개지던 그룹(임성현 등)의 공동비용 중복 자동 해소. (강제그룹/합치기 수기 없이도) 회사 사본 78그룹/61,812,000원, 과병합 0(조형만 가족만 캠퍼스 다름). v11-mergefee 위에 얹음.
- (직전 v11/sv7 — **합치기 추가요금 버그 수정**): 합치기/강제그룹으로 재편성한 그룹에 **객실 그룹가·그룹비·침구가 안 붙어 조회 시 등록비만** 보이던 버그 해결.
...
- 배포 확인: enrich 토스트 `[v12-roster]` / submit doGet `sv7-mergefee`.
