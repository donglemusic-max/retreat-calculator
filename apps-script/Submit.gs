...
var SUBMIT_VERSION = 'sv23-issueseed'; // 배포 확인용 (웹앱 URL을 브라우저로 열면 보임)
...
function _groupSummary_(sheet, col, width, gid) {
  ...
  var rows = []; for (var r = 1; r < n; r++) if (_gv_(vals[r], col.gid) === gid && !(col.ver >= 0 && _isUncounted_(_gv_(vals[r], col.ver)))) rows.push(vals[r]);
...
function _issueScan_(body, sheet, col, width) {
  ...
  var seed = (body && body.seed) || []; // 프론트가 보낸 큐레이션 이슈(의사결정 등)도 함께 등록
...
function _admin_(sheet, H, col, width) {
  ...
  for (var r = 1; r < n; r++) {
    ...
    if (col.ver >= 0 && _isUncounted_(_gv_(row, col.ver))) continue; // #16/#17 구·삭제 + 테스트 행 제외(관리자 목록·집계)
    out.push({
      row: r + 1, name: _gv_(row, col.name), gender: _gv_(row, col.gender), contact: _gv_(row, col.contact),
      email: _gv_(row, col.email), campus: _gv_(row, col.campus), deptLabel: _gv_(row, col.dept),
      roomLabel: _gv_(row, col.room), occLabel: _gv_(row, col.occ),
...
function _recalcGroupFull_(sheet, H, col, width, gid, override) {
  ...
  ord.forEach(function (r) {
    if (col.ver >= 0 && _isUncounted_(_gv_(vals[r], col.ver))) return; // 구/삭제 + 테스트 제외
  ...
  idxs.forEach(function (r) {
    ...
    if (col.ver >= 0 && _isUncounted_(_gv_(row, col.ver))) { // 구/삭제 + 테스트: 금액 0, 폼/구분값 보존
...
function _memberDelete_(body, sheet, H, col, width) {
  ...
  if (body.newRep && col.rep >= 0) {
    ...
    for (var r2 = 1; r2 < nn; r2++) if (_gv_(v2[r2], col.gid) === gid && !_isUncounted_(_gv_(v2[r2], col.ver))) sheet.getRange(r2 + 1, col.rep + 1).setValue(body.newRep);
  }
  var total = _recalcGroupFull_(sheet, H, col, width, gid);
...
function _groupSet_(body, sheet, H, col, width) {
  ...
  for (var gi = 1; gi < vals.length; gi++) if (_gv_(vals[gi], col.gid) === gid && !_isUncounted_(_gv_(vals[gi], col.ver))) {
  ...
  if (col.ver >= 0) {
    var liveIdx = []; for (var r = 1; r < n; r++) if (_gv_(vals[r], col.gid) === gid && !_isUncounted_(_gv_(vals[r], col.ver))) liveIdx.push(r);
