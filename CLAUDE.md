...
- enrich 그룹핑을 **이메일 ∪ 전화번호** union-find로 교체. 공동비용(객실/투숙) 그룹당 1회만 계상.
- ⚠️ **입금자명·명단은 병합 신호에서 제외**: 검증 결과 무관한 사람 과병합 유발(이혜란+성호민 등). 명단은 확인필요 플래그로만.
- lookup도 groupId ∪ 전화로 정리(명단 언급 확장 제거 — 비그룹 노출 방지). 임성현 4명 전화 공유라 정상 병합/조회.
- 실데이터 검증: **92→89그룹, 총액 65,758,000 → 65,386,000원**(임성현류 중복 −372,000), 과병합 0(조형만은 실제 가족).
- ⚠️ 사장님: **Migrate.gs 재붙여넣기+enrichSheet 재실행**(재그룹·총액보정) + **Submit.gs 재배포**.
