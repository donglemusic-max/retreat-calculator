...
// ── 결과 이메일(#18) 발신 설정 ──────────────────────────────
// ⚠️ 보낸사람 '주소'는 이 스크립트를 소유/승인한 계정의 Gmail로 고정됩니다(임의 주소 위조 불가).
//    아래로 '표시이름'과 '답장받을 주소'는 바꿀 수 있고, no-reply는 Workspace 계정에서만 동작합니다.
var MAIL_SENDER_NAME = '주님의교회 리트릿'; // 받는 분에게 보이는 보낸사람 이름
var MAIL_REPLY_TO = '';                     // 답장받을 주소 (예: 교회 대표메일). 비우면 발신계정으로 감
var MAIL_NO_REPLY = true;                   // true=Workspace면 no-reply 주소로 발송(개인 Gmail은 무시됨)
...
function _submit_(body, sheet, col, width) {
  ...
  try {
    if (body.email && body.email.indexOf('@') > 0) {
      ...
      if (MAIL_NO_REPLY) mailOpts.noReply = true; // Workspace 전용, 개인 Gmail은 무시
