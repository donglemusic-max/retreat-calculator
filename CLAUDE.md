...
### Phase 3b (다음) — 관리자 페이지
- 요구(통화/메시지): 한눈 대시보드 + 방배정(교회배정자) + 확인필요 + 리마인드(미제출/미입금).
- 설계 결정 대기: 관리자 인증(PIN 추천)/방배정 깊이(수동 vs 자동제안)/입금확인은 관리자 수기 컬럼.
- 예정 컬럼 추가: 입금확인, 배정방, 관리자메모. doPost에 action 'admin'(전체조회)·'adminUpdate'(셀 수정) 추가 예정.
